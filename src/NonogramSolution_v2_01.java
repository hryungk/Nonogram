...
public class NonogramSolution_v2_01 
{   
     ...
     */
    public NonogramSolution_v2_01(NonogramProblem newProblem)
    {        
    ...
    {           
        ...
        {   
            ...
            if (!solved)
            {
                makeTrueBetweenTruesOneApart(curArrayInfo, arrayAnswer);
                /* Check whether this array is solved. */
                solved = checkAllTrue(curArrayInfo, arrayAnswer, solved);
            }
            ...
            if (!solved)
            {
                ...
                {       
                    ...
                    {             
                        ...
                        if (kp < x && q <= e+1 &&  
                            (closedWithFalse(p, q, k, arrayAnswer) && fullOfTrue(p,q,arrayAnswer))) // But not when the section is closed with falses.
                        {
                    ...
                    {                    
                        ...
                        for (int i = 0; i < numTrueClusters; i++)
                        {
                            ...
                            boolean isValidTrue = sumFromX <= e - begIdxTrue[i] + 1;
                            foundValidLaterSameTrue = foundValidLaterSameTrue || (foundLaterSameTrue && isValidTrue);
                        } // end for                        
                        ...
                        for (int i = 0; i < numEmptyClusters; i++)
                        {
                            ...
                            boolean temp = foundLaterLargerEmpty && ((thisLocTrue == -1) || isAfterSmallerTrueCluster);
                        ...
                        for (int i = 0; i < numEmptyClusters; i++)
                        {
                            ...
                            if (curBegIdxEmpty+curEmptyClusterLen < k) // If any of empty cluster follows immediately after true, the empty cluster count may not be correct.
                        ...
                        {          
                            ...
                            if (eachTrueMatchNum)
                            {
                                int[][] trueRange = getTrueRange(curArrayInfo, arrayAnswer);
                        ...
                        if (!solved && (x > kp/2) && (kp >= x) && (i0 >= b) && !overlap && (sumToX > earlierLargerEmptyLen) &&// [4,1,3] _ _ _ _ _ _ _ _ _ _ O _ _ _ _
                            (!((sum - numT == x && a >= 2 && isBigForTwo) || (kp < numE && a == 1) || foundValidLaterSameTrue ||
                            (numEmptyClusters == a && emptyClusterLen[s] == x && numT==x) ||    // [1,1,2] _XXOXX_XOO
                            foundEarlierLargerEmpty || foundValidLaterLargerEmpty || foundThisSameTrue) || // [3] _ _ _ _ _ _ X _ _ _                             
                            (numEmptyClusters == a && a >= 2 && sumFromX > laterLargerEmptyLen && (laterLargerEmptyLen!= 1 && (!emptyBetweenO || (isEnclosed && !isBigForTwo && foundThisTrue)))) ||// [1,2,1] _ _ _ X _ _ X _ _ _ --> _ _ _ X O O X _ _ _ // [4,3,2] X _ _ _ O _ _ _ _ _ X _ _ _ _ --> X _ _ _ O _ _ O O _ X _ _ _ _ // [1,1,2] X _ X X O X X X X _ X O O _ X --> don't fill
                        ...
                        if (!solved && !(s > 0 && kp >= curArray[s-1])  && (x > kp)// || numT>getSumToX(s, curArrayInfo, 0))
                        ...
                        if (!solved)
                        {
                            solved = makeFarCellsFalse(x, p, q, s, curArrayInfo, numT, sum, 
                                                       solved, arrayAnswer);
                            /* Check whether this array is solved. */
                            solved = checkAllTrue(curArrayInfo, arrayAnswer, solved);
                        } // end if
                        ...
                        if (!solved && isPFalse(p, arrayAnswer) 
                            && arrayAnswer[p+1] == Status.True && (x != lenCurTrue)
                            && p != q && sum + (a-1) > kp
                            && (sumToX > earlierLargerEmptyLen)&& (sumFromX > thisLargerEmptyLen || x == xMin || foundEarlierLargerTrue)) 
                        {
                        ...
                        // [3, 3, 2] _ _ _ _ _ _ _ _ _ _ O O X _ _ --> Don't fill                        
                                ...
                                && (p != q) && (sum + (a-1) > kp)
                                && (sumToX > earlierLargerEmptyLen) && (sumFromX > thisLargerEmptyLen || x == xMin))
                        {                            
    ...
    private int[] getpq_conditional(Status[] arrayAnswer, ArrayInfo curArrayInfo, int s, int q)
    {
        ...
        else
        {
            ...
            int[] pq = getpq_general(curArrayInfo, s);
            int pGeneral = pq[0];
            if (p > sumFromX)// In case previous q would have gone too far
    ...
    // Make cells in this section that are farther than remainingT false
    public boolean makeFarCellsFalse(int x, int p, int q, int s, ArrayInfo curArrayInfo,
    ...
    {   
        ...
        {                        
            ...
            {  
                makeFalseBetweenTrues(p, q, s, curArrayInfo, arrayAnswer);
                //System.out.println("\t" + printCells(arrayAnswer)); 
    ...
    {        
        ...
        if (locCurTrue != -1)  // when there is true cell
        {
            ...
            if (!overlap && 
                ...
                (lenCurTrue == x && trueClusterLen.length == a) || 
                (lenCurTrue == x && s == a-1 && locCurTrue > sum && endCurTrue > e - (x + 1)) || 
                (lenCurTrue == x && foundLastTrueClusterLater) || 
                (lenCurTrue == x && (endCurTrue >= e - sumFromX) && !isAbsMin && !isBigForTwo)))
                // [1, 7] _ _ _ _ _ O _ _ O _ _ _ _ _ _ --> Don't know                
                // [4,2,2]_ _ _ _ _ _ _ _ O O _ _ _ _ _ --> _ _ _ _ _ _ _ X O O X _ _ _ _
                // [4,1,3] _ _ _ _ _ _ _ _ _ _ O _ _ _ _ --> don't know
                // [4,1,3] _ _ _ _ _ _ _ _ _ O _ _ _ _ _
                // [4,1,3] O O O O _ O _ O O O _ _ _ _ _
                // [4,1,3] _ _ _ _ _ O O O O _ O _ O O O
    ...
    {
        int locCurTrue = -1;   // Beginning index of True cluster (first one if there are multiple, -1 if none)
        ...
        while (i < trueClusterLen.length && !foundTrueCluster)                    
        {
            ...
            if (foundTrueCluster)
            {
                locCurTrue = begIdxTrue[i];
                lenCurTrue = trueClusterLen[i];
            } 
            else
                i++;
        } // end for
        int[] result = {locCurTrue, lenCurTrue};
        return result;
    ...
    private void makeFalseBetweenTrues(int p, int q, int s, ArrayInfo curArrayInfo, Status[] arrayAnswer)
    {       
        int b = curArrayInfo.getBeg();
        int e = curArrayInfo.getEnd();
        int a = curArrayInfo.getNum();
        ...
        else    // when number of true cluster != number of numbers in array
        {
            ...
            if (locCurTrue != -1 && q <= e && arrayAnswer[q] == Status.False && (s > 0 && curArray[s-1] < lenCurTrue))
            {
                System.out.println("\tMake far cells false.");
                for (int i = locCurTrue + lenCurTrue +remainingT; i < q; i++)   // Make false after the range   
                    arrayAnswer[i] = Status.False;         
            ...
            if (locCurTrue != -1 && p >= b && arrayAnswer[p] == Status.False && (s > 0 && kp < sumToX))
            {
                System.out.println("\tMake far cells false.");
    ...
    private void makeTrueBetweenTruesOneApart(ArrayInfo curArrayInfo, Status[] arrayAnswer)
    {           
        int a = curArrayInfo.getNum();
        int[] curArray = curArrayInfo.getArray();
        int[] maxInArray = findMax(curArray);
        int xMax = maxInArray[0];// Index of the largest number in the array
        int maxIdx = maxInArray[1];// Largest number in the array   
        ...
        for (int s = 0; s < a; s++)
        {
            ...
            if (numTrueThis >= 2)   // Consider only when there are multiple true clusters.
            {
                ...
                // True clusters must be bigger than numbers other than xMax.
                ...
                if ((lengthTwoTrue <= xMax) && allEmpty && (biggerThanAll || a < numTrueClusters || !isBigForTwo))
                {   
    ...
    } // end makeTrueBetweenTruesOneApart
    ...
    private int[][] getTrueRange(ArrayInfo curArrayInfo, Status[] arrayAnswer)
    {
        int a = curArrayInfo.getNum();
        int b = curArrayInfo.getBeg();
        int e = curArrayInfo.getEnd();
